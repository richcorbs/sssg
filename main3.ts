...
// import { zip } from "https://deno.land/x/nzip@v1.2.2/mod.ts";
...
async function register(domain: string) {
  ...
  const registrationResponse: Response = await fetch(
    "http://domains.local:8000/register?domain=" + domain,
    {
      method: "POST",
      body: JSON.stringify({}),
    },
...
async function deploy(
  ...
  // 2. Check if the config file has a "token" key
  // 2a. If it doesn't then register
  // 3. Check if the config file has a "staging" key
  ...
  if (!config.token) {
    // TODO: Register
    // curl -X POST domains.local:8000/register
    const registrationResponse: Response = await fetch(
      "http://domains.local:8000/register",
      {
        method: "POST",
        body: JSON.stringify({}),
      },
